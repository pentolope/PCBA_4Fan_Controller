...
#: One class only. Every net on this board shares one clearance requirement:
#: the highest potential anywhere on it is the input rail, and the fan supply
#: and the logic side are the same galvanic system, so there is no isolation
#: barrier to declare a second class for. Conductor width for the fan current
#: is a routing constraint, not a clearance one, and belongs with the layout.
NET_CLASSES = [{
    "name": "Default",
    "clearance": 0.15,
    "track_width": 0.25,
